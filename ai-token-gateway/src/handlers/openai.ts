// ============================================================
// OpenAI Protocol Handler: Proxy routes for OpenAI-compatible API
// ============================================================
 ...
 * Strategy: check the providers table for a default provider.
 * In the future this could use a model-to-provider mapping table.
 */
...
/**
 * Extract usage info from a non-streaming OpenAI chat completion response.
 */
...
/**
 * Forward a non-streaming request to the upstream provider and return the response.
 */
async function forwardNonStreaming(
  ...
  const url = `${providerBaseUrl}/v1/chat/completions`;
...
/**
 * Forward a streaming request to the upstream provider and pipe SSE chunks back.
...
/**
 * Fastify plugin that registers OpenAI-compatible proxy routes.
 * Routes:
 *   POST /openai/v1/chat/completions — proxy chat completions
 *   GET  /openai/v1/models           — proxy model listing
 */
export async function openaiRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get(
    ...
    async (
      ...
      try {
        const url = `${provider.apiBaseUrl}/v1/models`;
        const response = await undiciRequest(url, {
          method: 'GET',
          headers: {
        ...
        if (response.statusCode >= 400) {
          ...
          return reply.code(response.statusCode).send(responseBody);
        }
